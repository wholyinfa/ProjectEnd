...
Peek = [
	{
		class: "BreathinFragment",
		content : "The Breathing Fragment"
	},
	{
		class: "Portal JourNey",
		content : "Sail with me on a cute lil' journey"
	},
	{
		class: "Portal Artery",
		content : "Get to know the REAL me"
	},
	{
		class: "Portal BigMo",
		content : "How good do i do my job?"
	},
	{
		class: "QuickAccess",
		content : "Spaceship's Panel"
	},
	{
		class: "Ped Handle",
		content : "Spaceship's Pedal"
	},
	{
		class: "Rev Handle",
		content : "Reverse Thrust"
	},
	{
		class: "PathFinder",
		content : "Pathfinder"
	...
	{
		id: "Reach",
		content : "Reach selected/next part"
	},
	{
		id: "Ditch",
		content : "Go back to previous part"
	}
];
Projects = [
	{
...
// Pre Requisitions
  ...
  Mjaccuzi.set('#Temporary .BigMo .Jaccuzi', {scale:1, z:0.01});
   Mjaccuzi
	...
	.to('#Temporary .BigMo .Jaccuzi', .5, {rotation: -20,transformOrigin: '40% 60%',scale : 1.1,opacity: 1 ,ease: SlowMo.ease.config( 0.7, 0.9, false) });
		...
		// Emryonic Animation
	Emryo = []; EmryoLock = [false];
	$("#EmryoS > div").each(function(i){
		Emryo[i] = new TimelineMax({paused:true,repeat: -1});
		Dur = window.innerHeight/200;
		Per = Dur/100;
		Emryo[i]
			.fromTo($(this).children().first(), Dur, {y:"0%"}, {y:"-94%",ease: Power0.easeNone})
			.fromTo($(this).children().last(), Dur, {y:"100%"}, {y:"0%",ease: Power0.easeNone,delay:-Dur});
			$(this).children().first().css({height:"106%"});
		if( $(this).hasClass("active") ){ TweenMax.set($(this),{opacity:1}); Emryo[i].play(); EmryoLock[0] = Emryo[i]; }
	});
  ...
  PedalPrevent
	...
	DeckCloudFly.add(
		...
		TweenMax.fromTo("#DeckCloud > .Cloud", .1, {
			autoAlpha: 0
		}, {
			autoAlpha: .6
		}), 0
...
fixset(true)
	...
	$("#Skillometer .Core .Strikes").click(function(){
		...
		Emginashun.to([ActiveCore.find(".ForReactor"),$("#Skillometer .GeloV>div")], .15, {
			...
			autoAlpha: 0,
			ease:   ToughEase
		}).to([ActiveCore.find(".ForReactor"),$("#Skillometer .GeloV>div")], .8, {
			autoAlpha: 1,
			scale: 1,
			ease:   ToughEase,
...
/*		if( (typeof(CardSelect.isActive[T]) !== "undefined" && !CardSelect.isActive[T]) && (typeof(CardSelect.mdReset[T]) !== "undefined" && CardSelect.mdReset[T]) ){
	...
	// Peek Markers
	$(".BreathinFragment,.QuickAccess,.Pedal .Handle,.Reverse .Handle,.PathFinder,#Reach,#Ditch")
		.mouseenter(function(){ Peeker($(this)) })
		.mouseleave(function(){ Peeker($(this),true) });
	Pathfinder();
...
function SetPanel(PrevClass){
	...
	if( typeof(PanelSwitch.obj[Class]) === "undefined" ) {
		...
		AddToActive = PanelSwitch.obj[Class].isActive();
		if( !PanelSwitch.obj[Class].isActive() ){
			PanelSwitch.obj[Class].restart();
		}
	}
	$(PanelSwitch.stillactive).each(function(i){
		if( this.isActive() ){
			this.eventCallback("onComplete", null);
		}
	});
	if( !AddToActive ){
		PanelSwitch.stillactive.push(PanelSwitch.obj[Class]);
	}

	PanelSwitch.obj[Class].eventCallback("onComplete", function(){
		if( typeof(PanelSwitch.stillactive) !== "undefined" && PanelSwitch.stillactive.length ){
			$(PanelSwitch.stillactive).each(function(i){
				if( !this.isActive() && PanelSwitch.obj[Class] !== this ){
					this.restart().pause();
					PanelSwitch.stillactive.splice(i, 1);
				}
			});
		}
	});
		...
		if( t.hasClass(Portal[X]) && !Global.RottenStillActive ){
			...
			$("#EmryoS > div").each(function(i){
				if( $(this).hasClass("active") ){
					ActiveEmryo = Emryo[i];
					EmryoProgress = Emryo[i].progress();
					TweenMax.to($(this), 1 ,{autoAlpha: 0, onComplete: function(){
							ActiveEmryo.pause();
						}});
				}
				if( $(this).hasClass(Portal[X]) ){
					UpcomingEmryo = Emryo[i];
				}
				if( typeof(UpcomingEmryo) !== "undefined" &&
					$(UpcomingEmryo._first.target[0]).parent().hasClass("active") &&
					i == $(this).parent().children().length-1){
					UpcomingEmryo = Emryo[0];
				}
			});
			$("#EmryoS > .active").toggleClass("active");
			if( t.hasClass("active") ) {
				$("#EmryoS").find(".Default").toggleClass("active");
			}else{
				$("#EmryoS").find("." + Portal[X]).toggleClass("active");
			}
			TweenMax.to($("#EmryoS").find(".active"), 1 ,{autoAlpha: 1});
			UpcomingEmryo.progress(EmryoProgress).play();
			EmryoLock[0] = UpcomingEmryo;
			// ReSet fly clearance
			if( Global.Rotten ){
...
function DivisionSequence(reset,undone){
	...
	// Trial Start
	Ascs = [".TheStorm",".Analyzer > .Core",[".TheStorm > .Particles > div"]];
...
// Emryo Controll
function EmryoRun(state){
	if( state == true ){
		EmryoLock[0].resume(); TweenMax.to("#EmryoS",.5,{autoAlpha: .3});
	}
	if( state == false ){
		EmryoLock[0].pause(); TweenMax.to("#EmryoS",.5,{autoAlpha: 0});
	}
}
// Kill functions
...
function PathCallTo(){
	...
	TweenMax.to(CurrentA, .2, {color: Color, onComplete: function(){
...
function AssetForm(t){
	AssetHover(t.parent(),true);
	if( EnterStorm.isActive() ){ return; }
	if ( FormArrange.isActive() ) {
		if (FormArrange.reversed()) {
			if( StormForm[0] === t.parent().attr("class") ){
				FormArrange.reversed(!FormArrange.reversed());
				return;
			}
		} else {
			FormArrange.duration(.8).reverse();
			$(FormArrange._first.target[0]).find(".Title").toggleClass("active");
			if( StormForm[0] === t.parent().attr("class") ){ StormForm[0] = null; return; }
		}
	} else {
		if (FormArrange.progress() !== 0 && FormArrange.progress() === FormArrange.totalProgress()) {
			// Reversing the active asset
			FormArrange.reverse();
			TweenMax.set($(FormArrange._first.target[0]).find(".Submit"), {x: "0%"});
			$(FormArrange._first.target[0]).find(".Title").toggleClass("active");
			if( StormForm[0] === t.parent().attr("class") ){ StormForm[0] = null; return; }
		}
	...
	FormEase =  Back.easeInOut.config( 1.7);
	toY =  ( ( $("#SpaceCyclone").height() - ( (t.innerHeight() / .6) + TheForm.outerHeight() ) ) / 2 ) - Asset.offset().top ;

	TweenMax.set(TheForm.find(".Submit"), {x: "0%"});
	SubmitToY = ( ( ( $("#SpaceCyclone").height() * .9 ) - (TheForm.find(".Submit").offset().top + TheForm.find(".Submit").innerHeight()) ) * 100 ) / TheForm.innerHeight();
...
function ExitStorm(t){
	...
	// Resetting the forms
	if( StormForm[0] !== null ){
		AssetForm($("."+(StormForm[0]).replace(" ",".")).find(".Title"));
	}
	// Resetting the ritaul
	PostRitual.reverse();
		...
		if( CurrentParticle.hasClass(this.id) ){
			...
			$(this.pages).each(function(i){
				...
				if( this.active ){
					...
					singleparticle.find(".Cover").unbind("click").click(function(){
						...
						ExpandPreview.to("#AntiToxins .Preview .Cover", .2, {autoAlpha: 0}, 0)
							.to(singleparticle.find(".Preview > .img"), .2, {height: window.innerHeight,width: window.innerWidth,
								top: -singleparticle.find(".Preview > .img").offset().top,
								left: -singleparticle.find(".Preview > .img").offset().left,
								onComplete: function(){
									singleparticle.find(".Preview img").click(function(){
										ExpandPreview.reverse();
									});
								}}, 0);
					});
				}
				var ImageHref = this.image;
				PageContainer.children().last().click(function(){
					...
					Navigation.to(CoverChildren.last(), .1, {autoAlpha: 0}, 0);

					$(this).siblings(".active").removeClass("active");
					$(this).addClass("active");

					Navigation.to(singleparticle.find(".Preview > .img"), .4, {autoAlpha: 0, onComplete: function(){
							TweenMax.to(CoverChildren.last(), .1, {autoAlpha: 1});
							TweenMax.to(singleparticle.find(".Preview > .img"), .2, {autoAlpha: 1});
							singleparticle.find(".Preview img").attr("src", ImageHref);
						}}, 0);
				});
			});
...
// Peek setter
function Peeker(T,Del){
	if(Del){ Glitch.on("#Peek", null); return; }
	t = T; Content = null;
	for( X = 0 ; X < Peek.length ; X++ ) {
		if( t.attr("id") && t.attr("id") === Peek[X].id ){
			Content = Peek[X].content;
		}
		if( Peek[X].class ) {
			if( typeof(Peek[X].class) == "object" ){
				for( XX = 0 ; XX < Peek[X].class.length ; XX++ ) {
					if (t.hasClass(Peek[X].class[XX])) {
						Content = Peek[X].content;
					}
				}
			}else{
				if (t.hasClass(Peek[X].class)) {
					Content = Peek[X].content;
				}
			}
		}
